...
extern PSF_Font font18;    // Глобальна змінна шрифту PSF
...
static void draw_knob(int x_pos, int y_pos, float radius, float knobAngle, float knobValue,
    ...
    for (int i = 0; i <= 100; i += 10) {
        ...
        float textRadius = innerRadius + (font12.width * charCount) / 2 + 4;
        Vector2 textPos = { center.x + cosf(tickRad) * textRadius, center.y + sinf(tickRad) * textRadius };

        float lineWidth = charCount * (font12.width + spacing) - spacing;
        DrawPSFText(font12, textPos.x - lineWidth/2 + spacing, textPos.y - font12.height/2 + spacing, buf, 1, colorText);
    }
    ...
    float lineWidth = charCount * (font18.width + spacing) - spacing;
    float lineHeight = font18.height;
    ...
    // Малюємо фон прямокутника кольором colorText
...
int Gui_Knob_Channel(int channel, int x_pos, int y_pos, const char *textTop, const char *textRight,
    ...
    *value = changed;
    ...
    draw_knob(x_pos, y_pos, radius, knobAngles[channel], *value, minValue, maxValue, colorText);
    ...
    if (mouseOver && textTop && strlen(textTop) > 0) {
        ...
        for (int i = 0; i < lineCount; i++) {
            // float lineWidth = (float)font18.width * strlen(lines[i]) + (strlen(lines[i]) - 1) * spacing + padding;

            int charCount = utf8_strlen(lines[i]);
            float lineWidth = charCount * (font18.width + spacing) - spacing + padding;
            if (lineWidth > maxWidth) maxWidth = lineWidth;
        }

        // Висота одного рядка
        float lineHeight = (float)font18.height;
    ...
    //     DrawPSFText(font18, x_pos - textWidth / 2, bgY - 50, textTop, 1, colorText);
    // }

    // Відображення тексту справа
    if (textRight && strlen(textRight) > 0) {
        DrawPSFText(font18, x_pos + radius + 10, y_pos - 10, textRight, 1, colorText);
    }
