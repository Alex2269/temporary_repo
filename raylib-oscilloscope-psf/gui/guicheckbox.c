...
void Gui_CheckBox(Rectangle bounds, bool *checked, PSF_Font font, const char *textTop, const char *textRight, Color color)
{
    ...
    if(textRight != NULL && textRight[0] != '\0') {
        ...
        float textRightWidth = maxLineWidthCharsRight * (font.width + localSpacing) - localSpacing + 2 * padding;
        float textRightHeight = lineCountRight * font.height + (lineCountRight - 1) * LineSpacing + 2 * padding;
        ...
        DrawPSFText(font, textRightPos.x, textRightPos.y, textRight, localSpacing, textColor);
    }
