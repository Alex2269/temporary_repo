...
// Зовнішня функція малювання тексту PSF-шрифтом, реалізована поза цим файлом
...
// Функція для підрахунку кількості UTF-8 символів у рядку (не байтів!)
// Ігнорує байти продовження UTF-8 послідовності (0x80 – 0xBF)
// int utf8_strlen(const char* s)
// {
//     int len = 0;
//     while (*s) {
//         // Якщо це не байт продовження, рахунемо символ
//         if ((*s & 0xC0) != 0x80) len++;
//         s++;
//     }
//     return len;
...
// Функція, що змінює насиченість кольору (для підсвічування ручки)
static Color ChangeSaturation(Color c, float saturationScale)
{
    // Перекладаємо RGB в HSV
    float r = c.r / 255.0f;
    float g = c.g / 255.0f;
    float b = c.b / 255.0f;

    // Знаходимо максимум, мінімум значень RGB компоненти і їх різницю
    ...
    if (delta > 0) {
        // Обчислення відтінку залежно від найбільшої компоненти кольору
        if (cMax == r)
            ...
            h = (r - g) / delta + 4.0f;
        h *= 60.0f; // переводимо у градуси
        if (h < 0) h += 360.0f;
    }

    // Обчислення насиченості та яскравості (value)
    float s = (cMax == 0) ? 0 : delta / cMax;
    float v = cMax;

    // Змінюємо насиченість помноженням та обмежуємо у діапазон [0,1]
    s *= saturationScale;
    if (s > 1.0f) s = 1.0f;

    // Переклад HSV назад у RGB
    float cVal = v * s;
    ...
    newColor.a = c.a; // Зберігаємо альфа-канал
    return newColor;
}

// Отримуємо контрастний колір (чорний або білий) для читабельності на заданому фоні
    ...
    for (int i = slidersCount - 1; i >= 0; i--)
    {
        ...
        if (slider->isActive || IsMouseNearCircleKnob(mousePos, slider))
        {
            ...
            if (slider->textTop && slider->textTop[0] != '\0')
            {
                ...
                float lineHeightTop = (float)font.height;
            ...
            if (slider->textRight && slider->textRight[0] != '\0')
            {
                ...
                float lineHeightRight = (float)font.height;
            ...
            DrawPSFText(font, (int)(valueRect.x + padding), (int)(valueRect.y + padding / 2), valueText, localSpacing, textColorVal);
