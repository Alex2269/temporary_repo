...
void DrawCursorsAndDistance(Cursor *cursors, int count, PSF_Font font, int fontSize, DragRect *centerRect) {
    ...
    DrawPSFText(font, curPosA.x, curPosA.y, TextFormat("A:%i", cursors[0].value), spacing, cursors[0].color);
    Vector2 curPosB = {550, 10};
    DrawPSFText(font, curPosB.x, curPosB.y, TextFormat("B:%i", cursors[1].value), spacing, cursors[1].color);
}
