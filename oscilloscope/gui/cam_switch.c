...
// Функція обчислення яскравості кольору (luminance), використовується для вибору контрастного тексту
static float GetLuminance(Color color);
// Функція повертає білий або чорний колір залежно від яскравості фону
                           ...
                           float minValue, float maxValue, Color colorText)
{
    ...
    DrawRectangleLinesEx((Rectangle){rectX, rectY, rectWidth, rectHeight}, 1, Fade(GetContrastingTextColor(colorText), 0.5f));
                               ...
                               float *angle, bool isActive, int channel, float minValue, float maxValue)
{
    ...
    if (*dragging) {
        *angle = ang;
        ...
        // Нормалізуємо кут у діапазон від 0 до 1 для визначення проміжного значення
        float normalized = (*angle + 135.0f) / 270.0f;

        // Обчислюємо поточне значення rotary cam switch у заданому діапазоні
        value = minValue + normalized * (maxValue - minValue);

        // Обмеження значення в межах
                          ...
                          float radius, float *value, float minValue, float maxValue, bool isActive, Color colorText)
{
    ...
    DrawRoundedRectangle(bgX, bgY, bgWidth, bgHeight, 4, Fade(GetContrastingTextColor(colorText), 0.8f));
    ...
    *value = changed; // Оновлюємо зовнішнє значення
    ...
    if (!isDragging[channel]) {
        float roundedValue = roundf((*value - minValue) / valueStep) * valueStep + minValue;
        if (roundedValue < minValue) roundedValue = minValue;
        if (roundedValue > maxValue) roundedValue = maxValue;

        *value = roundedValue; // Оновлюємо значення після округлення

        int tickIndex = (int)roundf((roundedValue - minValue) / valueStep);
        camSwitchAngles[channel] = tickIndex * angleStep - 135.0f; // Оновлюємо кут для індикатора
    }
...
// Обчислення яскравості кольору (luminance) для визначення контрастності
static float GetLuminance(Color color) {
...
// Вибір контрастного кольору тексту (білий або чорний) у залежності від фону
static Color GetContrastingTextColor(Color bgColor) {
